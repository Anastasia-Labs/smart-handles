...
module SmartHandles (psmartHandleValidatorW) where
...
import PlutusLedgerApi.V1.Value (TokenName(..), CurrencySymbol(..))
import PlutusLedgerApi.V1 (Address(..), PubKeyHash(..), Credential(..), StakingCredential(..), ScriptHash)
import Plutarch.TryFrom (PTryFrom (PTryFromExcess, ptryFrom'))
import Utils (pelemAt', pand'List)
  ...
  PReclaim _ -> 
    pmatch (pfield @"credential" # (pfield @"owner" # dat)) $ \case 
      PPubKeyCredential ((pfield @"_0" #) -> pkh) -> 
        (pif (pelem @PBuiltinList # pkh # (pfield @"signatories" # (pfield @"txInfo" # ctx)))
             (pconstant ())
             perror
        )
      _ -> perror
...
data PRouterRedeemer (s :: S) = PRouterRedeemer
    ( Term
          s
          ( PDataRecord
...
psmartHandleSuccessor ::
  ...
  Term s PTxOut -> 
  Term s PInteger
psmartHandleSuccessor datums swapAddress foldCount smartInput swapOutput = unTermCont $ do 
    ...
    pmatch red $ \case 
      ...
      PReclaimSmart _ ->
        pmatch (pfield @"credential" # dat) $ \case 
          PPubKeyCredential ((pfield @"_0" #) -> pkh) -> 
            (pif (pelem @PBuiltinList # pkh # (pfield @"signatories" # ctxF.txInfo))
                 (popaque $ pconstant ())
                 perror
            )
